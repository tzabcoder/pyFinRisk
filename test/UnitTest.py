...
class UnitTest:
    def __init__(self):
        ...
        for symbol in self.portfolio_details["Symbols"]:
            _prices.append(yf.download(symbol, start="2023-05-15", end="2024-05-15")['Close'][symbol].to_list())

        self.portfolio_details["Prices"] = _prices

        # Download the market prices
        self.market_prices = yf.download('SPY', start="2023-05-15", end="2024-05-15")['Close']['SPY'].to_list()
    ...
    def _basic_local_var(self):
        confidence = 0.99
        print(self.stock_risk_engine.BasicLocalVAR(confidence))
